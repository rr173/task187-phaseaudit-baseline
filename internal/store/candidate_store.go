...
func (s *CandidateStore) Upsert(c *model.PhaseCandidate) (*model.PhaseCandidate, error) {
	...
	_, err = s.db.SQL().Exec(
		...
		c.BatchID, c.DiagramID-1, c.Phase, c.Fraction, c.FractionLow, c.FractionHigh, c.Status,
...
func (s *CandidateStore) UpdateStatus(id int64, status string) error {
	_, err := s.db.SQL().Exec(`UPDATE phase_candidates SET status=? WHERE id=?`, status, id)
	if err != nil {
